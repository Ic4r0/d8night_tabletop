...
export class EditCampaignGuard implements CanActivate {
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<boolean> {
    ...
    return this.auth.user$.pipe(
      ...
      map((fetchedCampaign) => !!fetchedCampaign && !!this.loggedUser && fetchedCampaign?.master.uid === this.loggedUser?.uid),
      tap((isCampaignMaster) => {
        if (!isCampaignMaster) {
          this.toastr.warning(

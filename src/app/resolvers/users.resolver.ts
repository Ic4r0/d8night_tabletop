...
@Injectable()
export default class UsersResolver implements Resolve<Array<User>> {
  ...
  resolve() {
    return this.usersApi.getUsersList().pipe(
      tap(list => {
        const noNotAuthUsers = list.filter(({auth}) => auth.basic && auth.player);
        this.store.dispatch(setListUsers({ list: noNotAuthUsers }));
      }),
      catchError((err) => {
